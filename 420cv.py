#!/usr/bin/env python3
import os
import sys
import subprocess
import glob
import platform
from pathlib import Path

class VideoConverter:
    def __init__(self):
        self.formats = ["mp4", "avi", "mov", "mkv", "webm", "flv", "wmv", "m4v", "3gp", "ogv", "ts", "vob", "asf", "rm", "rmvb", "f4v"]
        self.is_windows = platform.system() == "Windows"
        
    def show_banner(self):
        banner = """
╔═══════════════════════════════════════════════════════════╗
║                    🎬 420CV - FAST CONVERT 🎬             ║
║                                                           ║
║              Interactive Video Converter Shell            ║
║                    Cross-Platform Edition                 ║
╚═══════════════════════════════════════════════════════════╝
        """
        print(banner)
    
    def show_help(self):
        help_text = """
🚀 COMMANDS:
  video.avi mp4                         # Convert to MP4
  movie.mov webm                        # Convert to WebM  
  batch *.mp4 avi                       # Convert all MP4 to AVI
  formats                               # Show formats
  help                                  # Show this help
  exit                                  # Exit 420cv

🎯 FORMATS: mp4, avi, mov, mkv, webm, flv, wmv, m4v, 3gp, ogv, ts
        """
        print(help_text)
    
    def show_formats(self):
        print("\n🎯 POPULAR: mp4 (best), avi (quality), mov (apple), mkv (open), webm (web)")
        print("📱 MOBILE: 3gp, m4v, f4v")
        print("🌐 WEB: flv, ogv, ts")
        print("💻 LEGACY: wmv, asf, rm, rmvb, vob")
    
    def get_ffmpeg_cmd(self):
        """Get appropriate ffmpeg command for the platform"""
        if self.is_windows:
            # Try common Windows paths
            possible_paths = [
                "ffmpeg.exe",
                "C:\\ffmpeg\\bin\\ffmpeg.exe",
                "C:\\Program Files\\ffmpeg\\bin\\ffmpeg.exe"
            ]
            for path in possible_paths:
                try:
                    subprocess.run([path, "-version"], capture_output=True, check=True)
                    return path
                except (subprocess.CalledProcessError, FileNotFoundError):
                    continue
            return "ffmpeg.exe"  # Fallback
        else:
            return "ffmpeg"
    
    def convert_video(self, input_file, output_file):
        if not os.path.exists(input_file):
            print(f"❌ File not found: {input_file}")
            return False
        
        print(f"🔄 Converting: {input_file} → {output_file}")
        
        try:
            ffmpeg_cmd = self.get_ffmpeg_cmd()
            cmd = [ffmpeg_cmd, "-i", input_file, "-y", output_file, "-loglevel", "error", "-stats"]
            
            if self.is_windows:
                # Hide console window on Windows
                result = subprocess.run(cmd, creationflags=subprocess.CREATE_NO_WINDOW)
            else:
                result = subprocess.run(cmd)
            
            if result.returncode == 0:
                size = os.path.getsize(output_file) / (1024*1024)
                print(f"✅ Done! Size: {size:.1f}MB → {output_file}")
                return True
            else:
                print("❌ Conversion failed!")
                return False
                
        except FileNotFoundError:
            if self.is_windows:
                print("❌ ffmpeg not found! Download from: https://ffmpeg.org/download.html")
                print("   Extract to C:\\ffmpeg\\ or add to PATH")
            else:
                print("❌ ffmpeg not found! Install: sudo pacman -S ffmpeg")
            return False
    
    def batch_convert(self, pattern, output_format):
        files = glob.glob(pattern)
        
        if not files:
            print(f"❌ No files found: {pattern}")
            return
        
        print(f"🔄 Converting {len(files)} files to {output_format.upper()}")
        
        success = 0
        for i, file in enumerate(files, 1):
            input_path = Path(file)
            output_file = f"{input_path.stem}_converted.{output_format}"
            
            print(f"[{i}/{len(files)}] {input_path.name}")
            
            if self.convert_video(file, output_file):
                success += 1
        
        print(f"🎉 Complete! {success}/{len(files)} converted")
    
    def parse_command(self, cmd):
        parts = cmd.strip().split()
        
        if not parts:
            return
        
        if parts[0] == "help":
            self.show_help()
        elif parts[0] == "formats":
            self.show_formats()
        elif parts[0] == "exit":
            print("👋 Goodbye!")
            return False
        elif parts[0] == "batch" and len(parts) >= 3:
            self.batch_convert(parts[1], parts[2])
        elif len(parts) >= 2:
            input_file = parts[0]
            second_arg = parts[1]
            
            if second_arg.lower() in self.formats:
                input_path = Path(input_file)
                output_file = f"{input_path.stem}_converted.{second_arg}"
            else:
                output_file = second_arg
            
            self.convert_video(input_file, output_file)
        else:
            print("❌ Usage: <input> <format> | batch <pattern> <format> | help | exit")
        
        return True
    
    def run_interactive(self):
        self.show_banner()
        self.show_help()
        
        while True:
            try:
                cmd = input("\n420cv> ").strip()
                if not self.parse_command(cmd):
                    break
            except KeyboardInterrupt:
                print("\n👋 Goodbye!")
                break
            except EOFError:
                print("\n👋 Goodbye!")
                break

if __name__ == "__main__":
    converter = VideoConverter()
    converter.run_interactive()
